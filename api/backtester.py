...
def get_prices(tickers: list[str], start: date, end: date) -> pd.DataFrame:
    ...
    try:
        data = yf.download(
            syms, start=start.isoformat(), end=end.isoformat(),
    ...
    if len(syms) == 1:
        ...
        out = close.to_frame(name=syms[0]) if isinstance(close, pd.Series) else close
    else:
        if isinstance(data.columns, pd.MultiIndex):
    ...
    return out.dropna(how="all").ffill().bfill()
...
def run_backtest(
    ...
    for port in portfolios:
        ...
        w = np.array(weights, dtype=float) / (np.array(weights, dtype=float).sum())
        ...
        vals, dates = [starting_value], [pr.index[0]]
        cash, shares = starting_value, np.zeros(len(common))
        prev_rebal = pr.index[0]
        for i in range(1, len(pr)):
            dt, row, prev_row = pr.index[i], pr.iloc[i], pr.iloc[i - 1]
            if dt in rebal_dates or prev_rebal is None:
                ...
                new_shares = np.array([target_val[j] / prices_today[j] if prices_today[j] > 0 else 0 for j in range(len(common))])
        ...
        equity_curves[name] = [{"date": str(d.date()), "value": round(float(v), 2)} for d, v in equity.items()]
        ...
        sharpe = (cagr - 0.02) / vol if vol and vol > 0 else 0
    ...
    return {"equity_curves": equity_curves, "metrics": results, "error": None}
